...
def _format_result(result) -> str:
    lines = [
        ...
        f"_{result.proposed_action}_",
    ]
    if result.status == "awaiting_approval":
        lines.append(f"\nRequires approval — task `{result.task_id}`")
        lines.append(f"/approve {result.task_id} | /reject {result.task_id}")
...
class TelegramAdapter(ChannelAdapter):
    ...
    async def _cmd_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await update.message.reply_text(
            "D.R.A.K.E. is online.\n"
            "Send me any message, voice note, or URL and I'll handle it.\n\n"
    ...
    async def _cmd_approve(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await update.message.reply_text(f"Approving task {task_id[:8]}…")
    ...
    async def _cmd_correct(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not context.args or len(context.args) < 2:
            await update.message.reply_text("Usage: /correct <task_id> <new instruction>")
            return
        task_id = context.args[0]
    ...
    async def _cmd_projects(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        for p in projects:
            lines.append(f"• *{p['name']}* (`{p['id']}`) — {p['description'][:60]}")
        await update.message.reply_text("\n".join(lines), parse_mode="Markdown")

    async def _handle_text(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        uid = str(update.effective_user.id)
        if not self._is_allowed(uid):
            # Silent ignore — do not log message content for unauthorized users
    ...
    def _do_correct(self, task_id: str, correction: str) -> str:
        ...
        lines = [
            "*Re-interpreted:*",
            f"Intent: {result.intent} ({result.confidence:.0%} confident)",
            f"Proposed: _{result.proposed_action}_",
            f"/approve {task_id} | /reject {task_id}",
        ]
        return "\n".join(lines)
    ...
    def _do_task_detail(self, task_id: str) -> str:
        ...
        lines = [
            ...
            f"Action: _{task.requested_action[:120]}_",
        ]
        ...
        if task.status == "awaiting_approval":
            ...
            lines.append(f"/correct {task_id} <new instruction>")
        return "\n".join(lines)
