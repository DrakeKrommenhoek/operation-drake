...
"""
            path = self.artifact_service.save(
                title=f"Actions - {result.title}", content=md, task_id=task_id, artifact_type="action_list"
            )
            return WorkflowResult(
                success=True, artifact_path=path, summary=f"{len(result.action_items)} action items extracted"
            )
